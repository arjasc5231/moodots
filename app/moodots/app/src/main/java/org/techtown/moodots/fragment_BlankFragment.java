...
public class fragment_BlankFragment extends Fragment implements OnBackPressedListener{
    ...
    nadapter_DiaryAdapter_blank adapter;
    String voice;
    ...
    public void onBackPressed() {
        ...
        if(prev==1){
            ...
            fragment_bMainfrag mainfragment = new fragment_bMainfrag();//프래그먼트2 선언
            ...
            result.putString("bundleKey1", prevdata);
            FragmentTransaction transaction = getActivity().getSupportFragmentManager().beginTransaction();
            fragment_bSortfrag sortfragment = new fragment_bSortfrag();//프래그먼트2 선언
            ...
            FragmentTransaction transaction = getActivity().getSupportFragmentManager().beginTransaction();
            fragment_bSearchfrag searchfragment = new fragment_bSearchfrag();//프래그먼트2 선언
            ...
            result.putInt("bundelKey2", prev);
            FragmentTransaction transaction = getActivity().getSupportFragmentManager().beginTransaction();
            fragment_bSettingfrag settingfragment = new fragment_bSettingfrag();//프래그먼트2 선언
    ...
    public void initUI(ViewGroup rootView){
        ...
        adapter = new nadapter_DiaryAdapter_blank();
        ...
        adapter.setOnblankItemLongClickListener(new OnblankItemLongClickListener() {
            @Override
            public void onBlankItemLongClick(nadapter_DiaryAdapter_blank.ViewHolder holder, View view, int position) {
                zDiary item = adapter.getItem(position);
                Bundle result = new Bundle();
        ...
        if (getArguments() != null)
        {
            ...
            zAppConstants.println("debug bBlankfragment"+mMode);
            moodmod = getArguments().getInt("bundleKey2");
            Log.d("debug ", "debug mood  "+moodmod);
    ...
    private void saveDiary() {
        ...
        String sql = "insert into " + zDiaryDatabase.TABLE_DIARY +
    ...
    private void modifyDiary() {
        ...
        String sql = "UPDATE " + zDiaryDatabase.TABLE_DIARY +
                " SET " +
                " MOOD = '" + moodIndex + "'" +
                " ,CONTENTS = '" + scontents + "'" +
    ...
    private void deleteDiary() {
        ...
        // delete note
        String sql = "DELETE FROM " + zDiaryDatabase.TABLE_DIARY +
                " WHERE " +
                "_id = " + _id+";";
    ...
    public int loadDiaryListData(){
        ...
        String sql = "SELECT _id, MOOD, CONTENTS, HASHCONTENTS, CHECKMOD, DATE, TIME, VOICE FROM " + zDiaryDatabase.TABLE_DIARY +" WHERE CHECKMOD =0 ORDER BY _id DESC;";
        int recordCount= 0;
        zDiaryDatabase database = zDiaryDatabase.getInstance(context);
        if (database != null) {
            Log.d(TAG, sql);
